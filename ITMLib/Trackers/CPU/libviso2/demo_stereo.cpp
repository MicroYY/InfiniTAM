...
int main (int argc, char** argv) {
  ...
  for (int32_t i=0; i<373; i++) {
    ...
    try {
	  ...
	  cv::Mat left_img = cv::imread(left_img_file_name, -1);
	  cv::Mat right_img = cv::imread(right_img_file_name, -1);
	  ...
	  memcpy(left_img_data, left_img.data, width * height * sizeof(uint8_t));
	  memcpy(right_img_data, right_img.data, width * height * sizeof(uint8_t));
  ...
  cout << "Stereo: " << (double)(end - start) / CLOCKS_PER_SEC << "s" << endl;
